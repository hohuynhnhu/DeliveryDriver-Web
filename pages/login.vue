  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-glow-primary-50/30 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md">
      ...
      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-glow-primary-500 to-glow-primary-600 rounded-2xl shadow-2xl shadow-glow-primary-500/40 mb-4">
          <ShieldCheck v-if="isAdminMode" class="w-8 h-8 text-white" />
          <Package v-else class="w-8 h-8 text-white" />
        </div>
        <h1 class="text-3xl font-bold text-gray-900 mb-2">
          {{ isAdminMode ? 'Admin Portal' : 'Chào Mừng Trở Lại' }}
        </h1>
        <p class="text-gray-600">
          {{ isAdminMode ? 'Đăng nhập hệ thống quản trị' : 'Đăng nhập để tiếp tục sử dụng DeliveryDriver' }}
        </p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          ...
          <button
            type="submit"
            :disabled="isLoading"
            class="w-full bg-gradient-to-r from-glow-primary-500 to-glow-primary-600 text-white font-semibold py-3 rounded-xl shadow-lg shadow-glow-primary-500/30 hover:shadow-xl hover:shadow-glow-primary-500/40 transform hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
      ...
      <div v-if="!isAdminMode" class="mt-6 text-center">
        <p class="text-gray-600">
          ...
          <NuxtLink 
            to="/register" 
            class="text-glow-primary-600 hover:text-glow-primary-700 font-semibold inline-flex items-center gap-1 group"
          >
            Đăng ký ngay
            <ArrowRight class="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </NuxtLink>
